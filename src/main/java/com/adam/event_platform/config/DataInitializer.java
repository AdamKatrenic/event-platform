...
public class DataInitializer {
    ...
    CommandLineRunner initData() {
        return args -> {
            if (userRepository.findAll().stream().noneMatch(u -> u.getRoles().contains(Role.ADMIN))) {
                User admin = new User();
                admin.setUsername("admin");
                admin.setPassword(passwordEncoder.encode("admin123"));
                admin.setEmail("admin@example.com");
                admin.getRoles().add(Role.USER); // Start as user
                admin.getRoles().add(Role.ADMIN); // Make them admin immediately for testing setup
